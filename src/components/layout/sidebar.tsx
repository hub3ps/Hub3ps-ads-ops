...
function SidebarContent({
  ...
  const pathname = usePathname();
  const { accountName, displayName, loading, clients, selectedClientId } = useAccount();
  const isAll = selectedClientId === ALL_ACCOUNTS_ID;
  const selected = clients.find((c) => c.id === selectedClientId);
  ...
  return (
    <>
      {/* Logo */}
      <div className={cn("py-5 border-b border-[#e2e4ea]", collapsed ? "px-0 flex justify-center" : "px-5")}>
      ...
      <div className={cn("py-3 border-b border-[#e2e4ea]", collapsed ? "px-2 flex justify-center" : "px-3")}>
        {loading ? (
          ...
          <div
            className="w-10 h-10 rounded-lg bg-[#f5f6f8] flex items-center justify-center text-[12px] font-semibold text-[#111827] cursor-default"
            title={isAll ? "All accounts" : (selected?.name ?? accountName)}
          >
            {isAll ? (
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <rect x="1" y="1" width="5" height="5" rx="1.5" stroke="#4285F4" strokeWidth="1.2" />
                <rect x="8" y="1" width="5" height="5" rx="1.5" stroke="#4285F4" strokeWidth="1.2" />
                <rect x="1" y="8" width="5" height="5" rx="1.5" stroke="#4285F4" strokeWidth="1.2" />
                <rect x="8" y="8" width="5" height="5" rx="1.5" stroke="#4285F4" strokeWidth="1.2" opacity="0.4" />
              </svg>
            ) : (
      ...
      {/* Platform badges — hidden when collapsed */}
      {!collapsed && (
      ...
      <nav className="flex-1 px-3 py-4 flex flex-col gap-1 overflow-y-auto">
        <NavItem href="/dashboard"           label="Overview"         icon={<OverviewIcon />}       exact    collapsed={collapsed} />
        <NavItem href="/dashboard/insights"  label="Insights"         icon={<InsightsIcon />}                collapsed={collapsed} />

        {/* Campaigns — collapsed: single link; expanded: accordion */}
        {collapsed ? (
          <NavItem href="/dashboard/campaigns" label="Campaigns" icon={<CampaignsIcon />} collapsed={collapsed} />
        ) : (
          <div>
            <button
        ...
        <NavItem href="/dashboard/optimizations" label="Opt. History"    icon={<OptimizationsIcon />} collapsed={collapsed} />
        <NavItem href="/dashboard/profile"        label="Company Profile" icon={<ProfileIcon />}       collapsed={collapsed} />
      </nav>

      {/* Footer — collapse toggle (desktop only, not shown in mobile drawer) */}
      {onToggle && (
        <div className="px-3 py-3 border-t border-[#e2e4ea]">
          <button
            onClick={onToggle}
            className={cn(
              "w-full flex items-center gap-2.5 rounded-lg text-[13px] font-medium text-[#6b7280] hover:bg-[#f5f6f8] hover:text-[#374151] transition-colors",
              collapsed ? "justify-center px-2 py-2.5" : "px-3 py-2.5",
            )}
